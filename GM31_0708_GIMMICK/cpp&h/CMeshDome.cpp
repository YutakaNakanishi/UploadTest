...
//引数コンストラクタ
CMeshDome::CMeshDome(int nPriority,OBJTYPE Type):CScene(nPriority,Type)
{
	m_pD3DTex=NULL;
	m_pD3DVtxBuff=NULL;
	m_pDevice=NULL;
...
{}
//純粋仮想関数用
HRESULT CMeshDome::Init(CRenderer *pRenderer,D3DXVECTOR3 Pos,D3DXVECTOR3 Rot,char* filename,OBJTYPE Type)
{
	//書いてるだけで使いません！！
	return S_OK;
}
//メッシュ初期化
HRESULT CMeshDome::Init(CRenderer *pRenderer,int nNumBlockX, int nNumBlockZ, float fSizeBlockX, float fSizeBlockZ,D3DXVECTOR3 Pos,D3DXVECTOR3 Rot,float fRadius,char* filename)
{
...
//頂点情報のセット
void CMeshDome::SetVertexPolygon()
{
	//純粋仮想関数用なので使わない
}

//頂点情報のセット
void CMeshDome::SetVertexPolygon(int nNumBlockX, int nNumBlockZ, float fSizeBlockX, float fSizeBlockZ)
{
	//頂点情報計算用
	float fPosX,fPosY,fPosZ;
	float fTexU,fTexV;
	D3DXVECTOR3 fRot=D3DXVECTOR3(0.0f,D3DX_PI/2,0.0f);
	D3DXVECTOR3 fAddRot=D3DXVECTOR3(((2*D3DX_PI)/nNumBlockX),((D3DX_PI/2)/nNumBlockZ),0.0f);
	int nNum =0;
	VERTEX_3D *pVtx;
		//ロック
	...
	{
		fTexU = 0;
		fTexV = (float)i/(nNumBlockZ/4);
		for(int j = 0; j <= nNumBlockX; j++)
		{
			nNum = i * (nNumBlockX+1) + j;
			fPosX = float(m_radius*cos(fRot.y)*sin(fRot.x));
			fPosY = float(m_radius*sin(fRot.y));
			fPosZ = float(m_radius*cos(fRot.y)*cos(fRot.x));
			fTexU = (float)-j/(nNumBlockX/2);
...
CMeshDome*	CMeshDome::Create(CRenderer *pRenderer,int nNumBlockX, int nNumBlockZ, float fSizeBlockX, float fSizeBlockZ,D3DXVECTOR3 Pos,D3DXVECTOR3 Rot,float fRadius,char* filename,OBJTYPE Type)
{
	CMeshDome* pMeshDome=NULL;
	pMeshDome=new CMeshDome(3,Type);
	pMeshDome->Init(pRenderer,nNumBlockX,nNumBlockZ,fSizeBlockX,fSizeBlockZ,Pos,Rot,fRadius,filename);
	return pMeshDome;
}